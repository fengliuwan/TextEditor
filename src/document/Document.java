...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		char[] chArray = word.toLowerCase().toCharArray(); // returns lower case char array
		int count = 0;
		boolean newSyll = true;
		for(int i = 0; i < word.length(); i++) {
			if(i == word.length()-1 && chArray[i] == 'e' && newSyll && count >0) {
				break;
			}
			if(newSyll && isVowel(chArray[i])) {
				newSyll = false;
				count++;
			} 
			else if (!isVowel(chArray[i])) {
				newSyll = true;
			}	
		}
		return count;
	...
	/* my first versio that works, but uses string array that takes space
	protected int countSyllables(String word)
	{
		// TODO: Implement this method so that you can call it from the 
	    // getNumSyllables method in BasicDocument (module 2) and 
		...
		int num = syllables.length;
		int count = num;
		if (num > 0) {
			String lastWord = syllables[num-1];
			if(syllables[0].isEmpty()) {
				count--;
			if(lastWord.equals("e")) {
				if((num == 2 && !syllables[0].isEmpty()) || num > 2)
					count--;
				}
			}
		}
	...
	*/
	...
	public double getFleschScore()
	{
	    ...
	    if(numSent != 0) {
	    	score = score - 1.015 * ((double)numWords)/numSent - 84.6 * ((double)numSyll)/numWords;
	    }
	    return score;
	}

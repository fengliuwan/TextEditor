...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int i = 0; i <= s.length(); i ++) {
			for(int charCode = (int) 'a'; charCode <= (int)'z'; charCode++) {
				...
				if(!currentList.contains(sb.toString()) && !sb.toString().equals(s) && 
						(!wordsOnly || dict.isWord(sb.toString()))){
					currentList.add(sb.toString());
				}
			}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && retList.size() < numSuggestions && count <= THRESHOLD) {
			String curr = (String)((LinkedList)queue).remove();
			count++;
			for(String s : distanceOne(curr, false)) {
   ...
   public static void main(String[] args) {
	   ...
	   word = "kangoroo";
	   List<String> suggest1 = w.suggestions(word, 10);
	   System.out.println("Spelling Suggestions for \""+word+"\" are:");

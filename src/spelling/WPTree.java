...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		q.add(root);
		visitedList.add(word1);
		while(!q.isEmpty() && !visitedList.contains(word2)) {
			WPTreeNode curr = q.remove();
			List<String> nw = this.nw.distanceOne(curr.getWord(), true);
			for(String w : nw) {
				if(!visitedList.contains(w)) {
					WPTreeNode newNode = curr.addChild(w);
					q.add(newNode);
					visitedList.add(w);
					if(w.equals(word2)) {
	...
	private String printQueue(List<WPTreeNode> list) {
	   ...
	   public static void main(String[] args) {
		   ...
		   WPTree mytree = new WPTree(nw);
		   System.out.println(mytree.findPath("hello", "world"));
	   }
